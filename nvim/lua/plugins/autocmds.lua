...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function()
    ...
    -- Enable treesitter indent if parser is available
    local ok_get, ts_lang = pcall(vim.treesitter.language.get_lang, ft)
    if not ok_get then
      return
    end
    ts_lang = ts_lang or ft
    local ok_inspect, info = pcall(vim.treesitter.language.inspect, ts_lang)
    if ok_inspect and info and info.installed then
